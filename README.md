...
- **Publishes** to Supabase. The website in `docs/` (GitHub Pages) shows thumbnails, channel avatars, view counts, exact upload dates, and Official / Today / Series badges.
- **Mods** sign in with a shared password to queue, mark seen or skip, add links, and leave notes for tomorrow's run. Everyone else can view without an account.
...
collector/          daily job (Node 20+, no dependencies)
  ...
  lib/              youtube, reddit, feeds, rank (Claude + rules), store (Supabase), util
  test/             offline tests with fake APIs (npm test)
config/sources.json what to check: channels, searches, subreddits, feeds
supabase/schema.sql tables, read-only public access, password-checked mod actions
docs/               the website (index.html + config.js)
...
Privacy note: the website loads fonts from Google Fonts, site icons from Google's favicon service, and thumbnails from YouTube, so viewers' browsers contact those services.
